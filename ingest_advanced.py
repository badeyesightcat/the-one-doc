...
def get_file_metadata(file_path):
    ...
    meta = {"created_at": stats.st_birthtime, "author": "Unknown"}
...
def ingest_documents_advanced(folder_path):
    digital_library = []
    files = list(Path(folder_path).glob("*"))
    
    for file_path in files:
        if file_path.suffix not in [".pdf", ".docx"]: continue
